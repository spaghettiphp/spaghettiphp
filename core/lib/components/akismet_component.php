...
class AkismetComponent extends Component {
	...
	/**
	 * A shorter way to check whether a comment is a spam or not.
	 * SPAGHETTI CUSTOM IMPLEMENTATION
	 *
	 * @param array $params Comment param
	 * 
	 * @return bool	True is if the comment is spam, false if not.
	 */
	public function isSpam($params = array()){
		$this->wordPressAPIKey = $params["wordpress_api_key"];
		$this->setCommentAuthor($params["author_name"]);
		$this->setCommentAuthorEmail($params["author_email"]);
		$this->setCommentAuthorURL($params["author_url"]);
		$this->setCommentContent($params["comment"]);
		$this->setPermalink($params["permalink"]);
		
		if($this->isCommentSpam()):
			return true;
		else:
			return false;
		endif;
